                        ...
                        <div class="product-details-info">
                            <div class="product-detail">
                                <div class="details-info-top">
                                    ...
                                    <br/>
                                    @if (isset($product->pre_launch) && $product->pre_launch!="" )
                                        <span class="launching-time"> Lieferbar ab</span><br/>
                                        <b id="launchingTime" >{{ date("d.M-Y", strtotime($product->pre_launch))}}</b>
                                    @endif
                                        ...
                                        <div class="card">
                                            <div class="card-body">
                                                <div class="row">
                                                        ...
                                                        @endphp
                                                    <div class="col-md-18">
                                                        <label>Senden Sie mir eine E-Mail, sobald dieser Artikel vorrätig ist</label>
                                                    </div>
                                                    <div class="col-md-18">
                                                        
                                                        @if (isset($subscribes) && in_array($product->id, $subscribes))
                                                            <b>Product Already Subscribe</b>
                                                        @else
                                                            <form id="subscribeForm">
                                                                {{ csrf_field() }}
                                                                <input type="hidden" name="product_id" value="{{$product->id}}"/>
                                                                <input type="email" required name="subscribe_email" placeholder="Geben sie ihre E-Mailadresse ein" class="form-control"/>
                                                                <span class="text-danger" id="subscribe_email_msg"></span>
                                                                <br/>
                                                                <button class="btn btn-primary btn-block">Halten Sie mich auf dem Laufenden</button>
                                                            </form>
                                                        @endif
                                                    </div>
                                                </div>
                            ...
                            <div class="trust-content">
                                <ul>
                                    ...
                                    <li>
                                        Trusted Shops zertifiziert, sicheres Einkaufen und kostenlos versichert bis zu 100,- Euro
                                    </li>
                                    <li>
...
@push('scripts')
 ...
 <script>
    $("#subscribeForm").submit(function(e) {
          ...
          e.preventDefault();
               $.ajax({
                   url: "{{route('storefront.product.subscribe')}}",
                   type: "POST",
                   data: form_data,
                   async: false,
                   success: function (reponse) {
                           alert("Product Successfully Subscribe")
                           location.reload();
                   },error: function (xhr) {
                       setTimeout(function(){
